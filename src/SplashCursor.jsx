// SplashCursor — React Bits WebGL Navier-Stokes fluid simulation cursor overlay
// Source: https://reactbits.dev/backgrounds/splash-cursor  (JS + CSS variant)
// 'use client' removed — this is a Vite project, not Next.js
...
function SplashCursor({
  ...
  useEffect(() => {
    ...
    const themePalette = [
      '#e2723b', // Vibrant Terracotta
      '#9a3412', // Deep Burnt Orange
      '#78350f', // Dark Bronze
      '#9f1239', // Deep Crimson/Clay
      '#27272a'  // Dark Muted Slate/Olive
    ];
    ...
    function generateColor() {
      const hex = themePalette[Math.floor(Math.random() * themePalette.length)];
      return hexToRGB(hex);
    }
    ...
    function handleMouseDown(e) {
      let ptr = pointers[0];
      updatePointerDownData(ptr, -1, scaleByPixelRatio(e.clientX), scaleByPixelRatio(e.clientY));
      clickSplat(ptr);
    }
    ...
    function handleMouseMove(e) {
      let ptr = pointers[0];
      let px = scaleByPixelRatio(e.clientX), py = scaleByPixelRatio(e.clientY);
      if (!firstMove) { updatePointerMoveData(ptr, px, py, generateColor()); firstMove = true; }
      else { updatePointerMoveData(ptr, px, py, ptr.color); }
    }

    function handleTouchStart(e) {
      const t = e.targetTouches, ptr = pointers[0];
      for (let i = 0; i < t.length; i++)
        updatePointerDownData(ptr, t[i].identifier, scaleByPixelRatio(t[i].clientX), scaleByPixelRatio(t[i].clientY));
    }

    function handleTouchMove(e) {
      const t = e.targetTouches, ptr = pointers[0];
      for (let i = 0; i < t.length; i++)
        updatePointerMoveData(ptr, scaleByPixelRatio(t[i].clientX), scaleByPixelRatio(t[i].clientY), ptr.color);
    }

    function handleTouchEnd(e) {
      const t = e.changedTouches, ptr = pointers[0];
      for (let i = 0; i < t.length; i++) updatePointerUpData(ptr);
    }
